...
if (isset($_POST['btn-submit'])) {
    ...
    $q = mysqli_query($conn, "INSERT INTO produk VALUES('', '$gambar', '$nProduk', '$dProduk', '$hProduk');");
    if (mysqli_affected_rows($conn)) {
        echo "
            <script>
                window.location.href='produkTambah.php?a=y'
            </script>
        ";
    }
}
...
<body class="vh-100">
    ...
    <div class="w-100 d-flex" style="height: 65vh;">
        ...
        <main class="text-justify border border-dark p-4 position-absolute top-70 end-0 overflow-y-auto" style="width: 83%; height:65vh">
            <h3 class="mb-4">Tambah Produk Paleto Bay</h3>
            <?php if (isset($_GET['a'])) : ?>
                <p class="position-relative bg-info p-3 border border-info text-success text-center" style="--bs-bg-opacity: .4;">Produk Berhasil Ditambah <a href="produkKami.php" class="position-absolute d-block text-decoration-none text-danger bg-light rounded-circle border border-danger top-0 start-100 translate-middle" style="width: 28px;height: 28px;--bs-bg-opacity: .7;">x</a></p>
            <?php endif; ?>
            <form action="" method="POST">
                <div class="d-flex border-bottom mb-2">
                    <label for="gambar" class="w-50">Gambar</label>
                    <input type="file" name="gambar" id="gambar" class="form-control w-50" required>
                </div>
                <div class="d-flex border-bottom mb-2">
                    <label for="nProduk" class="w-50">Nama Produk</label>
                    <input type="text" name="nProduk" id="nProduk" class="form-control w-50" required>
                </div>
                <div class="d-flex border-bottom mb-2">
                    <label for="dProduk" class="w-50">Deskrpsi Produk</label>
                    <input type="text" name="dProduk" id="dProduk" class="form-control w-50" required>
                </div>
                <div class="d-flex border-bottom mb-2">
                    <label for="hProduk" class="w-50">Harga Produk</label>
                    <input type="number" name="hProduk" id="hProduk" class="form-control w-50" required>
                </div>
                <div class="d-flex gap-2 mb-2">
                    <a href="produkKami.php" class="w-50 btn btn-danger">Batal</a>
                    <button name="btn-submit" class="w-50 btn btn-primary">Simpan</button>
                </div>
            </form>
        </main>
    </div>
